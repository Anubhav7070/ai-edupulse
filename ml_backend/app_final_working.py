...
class FinalWorkingMLSystem:
    def __init__(self):
        self.scaler = StandardScaler()
        
    def analyze_dataset(self, df):
        """Dataset analysis with safe type conversion"""
        return {
    ...
    def train_models(self, X, y):
        """Train ML models with safe type conversion"""
        # Split data
        ...
        for name, model in models.items():
            try:
                ...
                if y_pred_proba is not None:
                    try:
                        roc_auc = safe_float(roc_auc_score(y_test, y_pred_proba, multi_class='ovr', average='weighted'))
                    except:
                        roc_auc = 0.0
                else:
                    roc_auc = 0.0
...
ml_system = FinalWorkingMLSystem()
